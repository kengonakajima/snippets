...
void on_read(uv_stream_t* stream, ssize_t nread, uv_buf_t buf) {
    ...
    if (nread >= 0) {
        //      fprintf(stderr, "data incoming:%d", (int)nread);
        ...
        fprintf(stderr, "data not incoming:%d", (int)nread);      
...
void after_write(uv_write_t* req, int status) {
    ...
    //    fprintf(stderr, "after_write: closing\n");
}
...
int main() {
    ...
    struct sockaddr_in address = uv_ip4_addr("0.0.0.0", 22280);
