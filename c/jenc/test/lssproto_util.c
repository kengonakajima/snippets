...
int lssproto_AllocateCommonWork(int bufsiz)
{
    ...
    lssproto.cryptwork = (char*)malloc( lssproto.workbufsize * 3 ); /* make it bigger */
    if( lssproto.cryptwork == NULL ){
        free( lssproto.work);free( lssproto.val_str);
        free( lssproto.escapework);free( lssproto.arraywork );free(lssproto.token_list);
        return -7;
    }
    lssproto_bzero( (char*)lssproto.cryptwork , lssproto.workbufsize * 3 );
