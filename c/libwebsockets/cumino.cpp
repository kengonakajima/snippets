...
void (*g_print_callback)(const char *msg) ;
...
void print( const char *fmt, ... ){
	...
	OutputDebugStringA("\n");
#endif
    if(g_print_callback) g_print_callback(dest);
}
...
// returns 0 if no file
double getFileModTime(const char *path) {
    struct stat sbuf;
    int r=stat(path,&sbuf);
    if(r<0)return 0;
    return (double)sbuf.st_mtime;    
}
